...
"""
This is python implementation on variable elimination problem for Q1.2.b
To test the program, compile the program with python3 vea1.py
And then uncomment the test examples at the end

factors in factorList in the format: eg.p(E) = p(e=t) + p(e=f)
[[E],[[1,0.5][0,0.5]]]
(hard code the factors in)

individual functions can be tested by uncommenting the test examples at the end except inference function
inference function can be tested by the other questions program(since the params will be hard coded)
"""
import numpy as np
...
def restrict(factor,variable,value):
    print("Restricting: ",variable," to be ",value)
    print("factor: ")
    print(factor)
    global n
    i = 0
...
def inference(factorList,queryVariables,orderedListOfHiddenVariables,evidenceList):
    ...
    while i < len_evi:
        ...
        while j < n:
            if evidenceList[i][0] in factorList[j][0]:
                restrict(factorList,evidenceList[i][0],evidenceList[i][1])
            j += 1
        i += 1
    #print restricted factor list
    print("Restricted factorlist: ")
    for f in factorList:
        print(f)
    ...
    while len(orderedListOfHiddenVariables) > 0:
        ...
        print("Summing out: ",current_hid_var)
        ...
        print("Summed out factor: ",temp)
        #print("factorList: ",factorList)
        factorList.append(temp)
        print("Summed out factorlist: ")
        for f in factorList:
            print(f)
...
#some initialization
fl = [[['S'],[[1,0.05],[0,0.95]]],
      [['B','S'],[[1,1,0.6],[1,0,0.1],[0,1,0.4],[0,0,0.9]]],
      [['M'],[[1,1/28],[0,27/28]]],
      [['NA'],[[1,3/10],[0,7/10]]],
      [['NH','M','NA'],[[1,1,1,0.8],[1,1,0,0.4],[1,0,1,0.5],[1,0,0,0],
                         [0,1,1,0.2],[0,1,0,0.6],[0,0,1,0.5],[0,0,0,1]]],
      [['FH','S','M','NH'],[[1,1,1,1,0.99],[1,1,1,0,0.9],[1,1,0,1,0.75],[1,1,0,0,0.5],
